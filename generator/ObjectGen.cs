	...
	public class ObjectGen : ObjectBase  {
		...
		public override void Generate (GenerationInfo gen_info)
		{
			...
			foreach (string attr in custom_attrs)
				sw.WriteLine ("\t" + attr);
			sw.WriteLine ("\t{0} {1}partial class " + Name, IsInternal ? "internal" : "public", IsAbstract ? "abstract " : "");
			...
			foreach (string iface in interfaces) {
				if (Parent != null && Parent.Implements (iface))
					continue;
				var ifaceGen = table.GetClassGen (iface) as InterfaceGen;
				if (ifaceGen.Version != null) {
					Utils.GenerateVersionIf (sw, ifaceGen.Version);
				}

				sw.WriteLine ("\t\t, " + table.GetCSType (iface));
				if (ifaceGen.Version != null) {
			...
			if (interfaces.Count != 0) {
				...
				foreach (string iface in interfaces) {
					if (Parent != null && Parent.Implements (iface))
						continue;
					InterfaceGen igen = table.GetClassGen (iface) as InterfaceGen;
					if (igen.Version != null) {
						Utils.GenerateVersionIf (sw, igen.Version);
					}
