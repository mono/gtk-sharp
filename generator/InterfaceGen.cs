...
namespace GtkSharp.Generation {
	...
	public class InterfaceGen : ObjectBase {
		...
		// these are to still generate the old interface names of the 2.x days (lacking the "I" prefix)
		public string DeprecatedName { get { return base.Name; } }
		public string DeprecatedImplementorName { get { return base.Name + "Implementor"; } }
		...
		void GenerateImplementorIface (GenerationInfo gen_info)
		{
			...
			if (!IsInternal) {
				sw.WriteLine ();
				sw.WriteLine ("\t[Obsolete (\"Use interface {0} instead\")]", ImplementorName);
				sw.WriteLine ("\tpublic interface " + DeprecatedImplementorName + " : " + ImplementorName + " { }");
				sw.WriteLine ();
			}
		...
		public override void Generate (GenerationInfo gen_info)
		{
			...
			if (!IsInternal) {
				sw.WriteLine ();
				sw.WriteLine ("\t[Obsolete (\"Use interface {0} instead\")]", Name);
				sw.WriteLine ("\tpublic interface " + DeprecatedName + " : " + Name + " { }");
				sw.WriteLine ();
			}
