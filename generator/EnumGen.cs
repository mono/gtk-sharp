	...
	public class EnumGen : GenBase {
		...
		public EnumGen (XmlElement ns, XmlElement elem) : base (ns, elem)
		{
			foreach (XmlElement member in elem.ChildNodes) {
				...
				string result = "\t\t" + member.GetAttribute(Constants.Name);
				if (member.HasAttribute(Constants.Value)) {
					string value = member.GetAttribute(Constants.Value);
					if (value.EndsWith("U")) {
						enum_type = " : uint";
		...
		public override void Generate (GenerationInfo gen_info)
		{
			...
			if (Elem.GetAttribute(Constants.Type) == Constants.Flags)
				sw.WriteLine ("\t[Flags]");
			if (Elem.HasAttribute(Constants.GType))
				sw.WriteLine ("\t[{0}]", Name);
			...
			if (Elem.HasAttribute (Constants.GType)) {
				AttributeHelper.Gen (sw, Name, LibraryName, Elem.GetAttribute (Constants.GType));
			}
